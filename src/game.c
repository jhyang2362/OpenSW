...
static void process_player(PacmanGame *game,int pacman_num);
...
void game_tick(PacmanGame *game)
{
	...
	switch (game->gameState)
	{
		...
		case GamePlayState:
			...
			if (game->pacman[0].score > game->highscore) game->highscore = game->pacman[0].score;
...
static void process_player(PacmanGame *game , int pacman_num)
{
	Pacman *pacman = &game->pacman[pacman_num];
	...
	//키 눌린게 있으면 true 반환됨
	bool dirPressed = dir_pressed_now(&newDir,pacman_num);
...
static void process_ghosts(PacmanGame *game)
{
	for (int i = 0; i < 4; i++)
	{
		...
		if (result == NewSquare)
		{
			...
			execute_ghost_logic(g, g->ghostType, &game->ghosts[0], &game->pacman[0]);
...
static bool resolve_telesquare(PhysicsBody *body)
{
	...
	if (body->y == 3 || body->y == 14 || body->y == 28)
	{
